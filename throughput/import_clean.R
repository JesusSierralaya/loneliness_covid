...
# IMPORT VARIABLES -------------------------------------------------------------
  ...
  ## MARITAL STATUS PRE -------------------------------------------------------
    ...
    db_maritalstatus <- PATH_FILE_PRE %>%
      read_dta(col_select = all_of(c("ID_ECS", # Id
                                     "q1012_mar_stat")))
  ...
  ## DISABILITY (WHODAS12) PRE-POST -------------------------------------------
    ...
    disability_pre <- PATH_PRE_DISABILITY %>%
      read.dta13(select.cols = all_of(c("ID_ECS", "whodas12"))) %>% 
  ...
  ## SOCIAL SUPPORT PRE-POST -------------------------------------------------
    ...
    socialsupport_pre <- PATH_FILE_PRE %>% 
      read.dta13(select.cols = all_of(c("ID_ECS",
                                     "q6320_close", 
                                     "q6310_help_neig", 
                                     "q6330_concern")))
    
    socialsupport_post <- PATH_FILE_POST %>% 
      read.dta13(select.cols = all_of(c("ID_ECS", 
                                     "SOLO9A", 
                                     "SOLO9C", 
                                     "SOLO9B")))
    ...
    # Transformation 
    socialsupport_pre %<>%
      mutate(socialsupport_pre = 
               q6320_close + (6 - q6310_help_neig) + (6 - q6330_concern), 
             .keep = "unused") # delete the column no longer need
    
    socialsupport_post %<>%
      mutate(socialsupport_post = 
               SOLO9A + (6 - SOLO9C) + (6 - SOLO9B), 
             .keep = "unused") # delete the column no longer need
  
    # Merge
    db_social_support <- full_join(socialsupport_pre, socialsupport_post,
                               by = "ID_ECS")
  ...
  ## VIRTUAL CONTACT (SOLO2) POST ----------------------------------------------
    ...
    db_virtualcontact_post <- PATH_FILE_POST %>%
      read.dta13(select.cols = all_of(c("ID_ECS", "SOLO2")))
    
    ### TRANSFORMATION ---
    
    db_virtualcontact_post %<>% 
      mutate(virtualcontact_post = SOLO2 %>% recode_factor(
        `1` = "Daily",
        `2` = "Once a week",
  ...
  ## SOCIAL RELATIONSHIP CHANGES (SOLO3) POST ----------------------------------
    ...
    db_socialchanges_post <- PATH_FILE_POST %>%
      read.dta13(select.cols = all_of(c("ID_ECS", "SOLO3")))
    
    ### TRANSFORMATION ---
    
    db_socialchanges_post %<>%
      mutate(socialchanges_post = SOLO3 %>% recode_factor(
        `1` = "Improved",
        `2` = "Worsened",
        `3` = "No"
      ), .keep = "unused")
  ...
  ## ECONOMY WORSENED POST ---------------------------------------------------
    ...
    db_economy %<>%
      mutate(economyworsened_post = economy %>% recode_factor(
        `1` = "Yes",
        `0` = "No"
      ), .keep = "unused")
...
# MERGE ------------------------------------------------------------------------
  ...
  DB_pre_post %<>% 
    relocate(ID_ECS, weights, age, sex, educlevel, maritalstatus, 
             virtualcontact_post, socialchanges_post, 
             economyworsened_post, unemployment_post, 
             materialdeprivation_pre, livingalone_post, 
             physicalactivity_pre, physicalactivity_post,
             depression_pre, depression_post,
             neuroticism_pre, extraversion_pre)
...
# DATABASES LONGER --------------------------------------
  ...
  DB_longer <- DB_pre_post %>% 
    ...
    select(ID_ECS, weights, 
         age_pre = age,
         sex_pre = sex, 
         educlevel_pre = educlevel,
         maritalstatus_pre = maritalstatus, everything()) %>% 
    pivot_longer(
    ...
    mutate(Time = Time %>% recode_factor(
      pre = "Before", post = "During", .ordered = TRUE
    ))
