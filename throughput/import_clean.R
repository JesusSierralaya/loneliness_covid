...
db_sex %<>%
  mutate(sex = q1009_sex %>% recode_factor(`1` = "Male",
                                           `2` = "Female"),
         .keep = "unused")
...
socialsupport_pre %<>%
  mutate(
    ...
    .keep = "unused"
  ) # delete the column no longer need
...
db_virtualcontact_post %<>%
  mutate(
    virtualcontact_post = SOLO2 %>% recode_factor(
      `1` = "Daily",
      `2` = "Once a week",
